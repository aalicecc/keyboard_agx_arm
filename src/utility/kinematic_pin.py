...
class Arm_IK:
    ...
    def __init__(self, urdf_path: str = None):
        ...
        try:
            urdf_path = self.resolve_urdf_with_xacrodoc(urdf_path)
            ...
            if urdf_path and os.path.exists(urdf_path):
                try:
                    os.unlink(urdf_path)
                    print(f"Temporary file: {urdf_path} has been deleted.")
                except Exception as del_err:
                    print(f"Deleting temporary file {urdf_path} failed: {del_err}")
    ...
    def resolve_urdf_with_xacrodoc(self, urdf_path):
        doc = XacroDoc.from_file(urdf_path)
        
        resolved_urdf_string = doc.to_urdf_string()
        
        with tempfile.NamedTemporaryFile(mode='w', suffix='.urdf', delete=False) as f:
            f.write(resolved_urdf_string)
